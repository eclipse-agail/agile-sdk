...
.then(function(data) {
  owner = data.id;
  return Promise.all([agile.idm.entity.deleteEntity("MyAgileClient2","client")]);
}).then(function(deletions){
    return agile.idm.entity.createEntity("MyAgileClient2","client",{"name":"MyAgileClient2","clientSecret":"theclientsecret", "redirectURI": "http://localhost:8080/auth/example/callback"});
  },function error(err){
    return agile.idm.entity.createEntity("MyAgileClient2","client",{"name":"MyAgileClient2","clientSecret":"theclientsecret", "redirectURI": "http://localhost:8080/auth/example/callback"});
  ...
  return agile.idm.entity.deleteEntity("MyAgileClient2","client");
